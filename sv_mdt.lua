...
Framework.Commands.Add("mdt", "Toggle Police MDT", {}, false, function(source, args)
	local src = source
	local Player = Framework.Functions.GetPlayer(source)
...
AddEventHandler("mdt:performOffenderSearch", function(query)
	...
	Framework.Functions.ExecuteSql(false, 'SELECT * FROM `characters_metadata` WHERE `citizenid` = "'..query..'" OR `charinfo` LIKE "%'..query..'%"', function(result)
...
AddEventHandler("mdt:performForensicsSearch", function(query)
	local src = source
	local matches = {}
	exports['ghmattimysql']:execute("SELECT characters_metadata.cid, characters_metadata.citizenid, characters_metadata.charinfo, user_mdt.fingerprint, user_mdt.notes FROM characters_metadata INNER JOIN user_mdt ON characters_metadata.citizenid = user_mdt.char_id", {
...
AddEventHandler("mdt:getOffenderDetailsById", function(char_id)
	local src = source
	exports['ghmattimysql']:execute('SELECT * FROM `characters_metadata` WHERE `citizenid` = @id', {
		['@id'] = char_id
	}, function(result)
...
AddEventHandler('mdt:server:BillPlayer', function(playerId, price)
    ...
    if Player.PlayerData.job.name == "police" then
        if OtherPlayer ~= nil then
            ...
            TriggerClientEvent('Framework:Notify', OtherPlayer.PlayerData.source, "You received a fine of $"..price)
        end
    end
end)
...
AddEventHandler("mdt:server:getPlayer", function(playerid)
	...
	if playerid ~= nil then 
		_citizenID = Framework.Functions.GetPlayer(playerid)
	end
...
AddEventHandler("mdt:sentencePlayer", function( jailtime, charges, char_id, fine, characters_metadata)
	...
	for _, src in pairs(characters_metadata) do
		if src ~= 0 and GetPlayerName(src) then
			exports['ghmattimysql']:execute('SELECT * FROM `characters_metadata` WHERE `citizenid` = @identifier', {
				['@identifier'] = char_id
			}, function(result)
...
AddEventHandler("mdt:getVehicle", function(vehicle)
	local src = source
	exports['ghmattimysql']:execute("SELECT * FROM `characters_metadata` WHERE `citizenid` = @query", {
...
function GetLicenses(identifier, cb)
	local Player = Framework.Functions.GetPlayer(identifier)
